...
// --- from Blockly/math.js ---

Blockly.Arduino['number'] = function(block) {
  var code = window.parseFloat((block || this).getFieldValue('NUM'));
  return [code, Blockly.Arduino.ORDER_ATOMIC];
};

Blockly.Arduino.math_number = function() {
  // Numeric value. Guard against missing field (e.g. shadow block or load order).
  var num = this.getFieldValue('NUM');
  var code = (num !== undefined && num !== null && num !== '') ? window.parseFloat(num) : 0;
...
Blockly.Arduino['math_random_float'] = function() {
  ...
  var functionName = Blockly.Arduino.provideFunction_(
      ...
      [ 'float ' + Blockly.Arduino.FUNCTION_NAME_PLACEHOLDER_ +
        ...
        '}\n']);
  return [functionName + '()', Blockly.Arduino.ORDER_FUNCTION_CALL];
};

...
Blockly.Arduino.inout_digital_write = function() {
  var dropdown_pin = this.getFieldValue('PIN') || '13';
  var dropdown_stat = this.getFieldValue('STAT') || 'HIGH';
...
Blockly.Arduino.inout_PWM_write = function(block) {
  var pin = block.getFieldValue('PIN') || '9';
  var value = Blockly.Arduino.valueToCode(block, 'NUM', Blockly.Arduino.ORDER_ATOMIC) || '0';
  Blockly.Arduino.setups_['setup_output_' + pin] = 'pinMode(' + pin + ', OUTPUT);';
  // 0–100% → 0–255 for analogWrite
  var code = 'analogWrite(' + pin + ', constrain((' + value + ' * 255) / 100, 0, 255));\n';
  return code;
};

Blockly.Arduino.inout_PWM_write_validator = function() {
  var dropdown_pin = this.getFieldValue('PIN') || '9';
  var dropdown_stat = this.getFieldValue('STAT') || 'HIGH';
  Blockly.Arduino.setups_['setup_output_' + dropdown_pin] = 'pinMode(' + dropdown_pin + ', OUTPUT);';
  var code = 'digitalWrite(' + dropdown_pin + ', ' + dropdown_stat + ');\n';
  return code;
...
Blockly.Arduino.inout_analog_write_validator = function(block) {
  var pin = block ? block.getFieldValue('PIN') : this.getFieldValue('PIN');
  var value = block ? Blockly.Arduino.valueToCode(block, 'NUM', Blockly.Arduino.ORDER_ATOMIC) || 0 : this.getFieldValue('NUM');
  Blockly.Arduino.setups_['setup_output_' + pin] = 'pinMode(' + pin + ', OUTPUT);';
  var code = 'analogWrite(' + pin + ', ' + value + ');\n';
  return code;
};

Blockly.Arduino.inout_PWM_write_inline = function(block) {
  var pin = block.getFieldValue('PIN');
  var pct = Number(block.getFieldValue('NUM')) || 0;
  Blockly.Arduino.setups_['setup_output_' + pin] = 'pinMode(' + pin + ', OUTPUT);';
  var value = Math.round((pct * 255) / 100);
...
Blockly.Arduino.inout_digital_read = function() {
  var dropdown_pin = this.getFieldValue('PIN') || '2';
  // Removed automatic pinMode - user should set pin mode manually if needed
...
Blockly.Arduino.inout_analog_read = function() {
  // Block uses a FieldDropdown named 'PIN'
  var dropdown_pin = this.getFieldValue('PIN') || 'A0';
...
Blockly.Arduino.inout_analog_read_validator = function() {
  var dropdown_pin = this.getFieldValue('PIN') || 'A0';
  // Accept either numeric GPIO (e.g., 33) or Arduino-style tokens (e.g., A0)
  var code = 'analogRead(' + dropdown_pin + ')';
  return [code, Blockly.Arduino.ORDER_ATOMIC];
};

Blockly.Arduino.inout_analog_read_voltage = function() {
  var dropdown_pin = this.getFieldValue('PIN') || 'A0';
  // Resolution and reference depend on board; safe fallback when profile missing
  var isEsp32 = (typeof profile !== 'undefined' && profile.defaultBoardKey && String(profile.defaultBoardKey).indexOf('esp32') !== -1);
  var ref = isEsp32 ? 3.3 : 5.0;
  var res = isEsp32 ? 4095.0 : 1023.0;
  var code = '(analogRead(' + dropdown_pin + ') * ' + ref + ' / ' + res + ')';
  return [code, Blockly.Arduino.ORDER_ATOMIC];
};

Blockly.Arduino.inout_onoff = function() {
  // Boolean values HIGH and LOW. Safe for use inside if/loop (e.g. Built-in LED block).
...
Blockly.Arduino.tone_notime = function() {
  var value_pin = this.getFieldValue('PIN') || '8';
  var value_num = Blockly.Arduino.valueToCode(this, 'NUM', Blockly.Arduino.ORDER_ATOMIC) || '440';
  Blockly.Arduino.setups_['setup_output_'+value_pin] = 'pinMode('+value_pin+', OUTPUT);';
...
Blockly.Arduino.inout_attachInterrupt = function() {
  var dropdown_pin = this.getFieldValue('PIN') || '2';
  var dropdown_mode = this.getFieldValue('mode') || 'CHANGE';

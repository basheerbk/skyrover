...
Blockly.Arduino.base_setup_loop = function () {
  ...
  var loop = Blockly.Arduino.statementToCode(this, 'LOOP');
  var setup_key = Blockly.Arduino.variableDB_.getDistinctName('base_setup', Blockly.Variables.NAME_TYPE);
  Blockly.Arduino.setups_[setup_key] = branch || '';
  return loop || '';
};
...
Blockly.Arduino.base_delay_sec = function () {
  ...
  // Multiplication done in C++ so expressions (e.g. a variable or sensor value) work correctly
  var code = 'delay((long)(' + delay_time + ') * 1000L);\n';
  return code;
};
...
Blockly.Arduino.base_map = function () {
  ...
  var value_dmax = Blockly.Arduino.valueToCode(this, 'DMAX', Blockly.Arduino.ORDER_ATOMIC) || '255';
...
Blockly.Arduino.inout_digital_mode = function () {
  ...
  var dropdown_mode = this.getFieldValue('PINMODE');
...
Blockly.Arduino.inout_buildin_led = function (block) {
  ...
  Blockly.Arduino.setups_['setup_output_' + ledPin] = 'pinMode(' + ledPin + ', OUTPUT);';
  var code = 'digitalWrite(' + ledPin + ', ' + dropdown_stat + ');\n';
  return code;
};
